...
| `REJECTED / RESOLVED` | Comparison with the final scene showed that the proposal duplicates or is already satisfied by existing screenplay action or dialogue. |
...
### Stage 3 — Short character and emotion beats: complete

- `VENPA-USE-001` — approved and implemented
- `VENPA-USE-004` — rejected because Scene `#7#` already carries the doorway-witness action
